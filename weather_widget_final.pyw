...
class WeatherWidget(QMainWindow):
    ...
    def translate_weather(self, description):
        ...
        if desc_lower in translations:
            return translations[desc_lower]  # Vraća tuple (text, emoji)
    ...
    def getUVColor(self, uv_value):
        ...
        if uv_value < 3:
            return "#4CAF50"  # Zelena - nizak
        elif uv_value < 6:
            return "#FFEB3B"  # Žuta - umereno
        elif uv_value < 8:
            return "#FF9800"  # Narandžasta - visok
        elif uv_value < 11:
            return "#F44336"  # Crvena - veoma visok
        else:
            return "#9C27B0"  # Ljubičasta - ekstremno
    
    def getAQIColor(self, aqi_index):
        """Vrati boju za Air Quality Index"""
        colors = {
            1: "#4CAF50",  # Odličan - zelena
            2: "#8BC34A",  # Dobar - svetlo zelena
            3: "#FFEB3B",  # Srednji - žuta
            4: "#FF9800",  # Loš - narandžasta
            5: "#F44336"   # Veoma loš - crvena
        }
        return colors.get(aqi_index, "#999999")
    
    def changeWidgetSize(self, width, height, label):
        """Promeni veličinu widgeta"""
        # Očisti sve checkmark-ove
        for action in self.size_actions.values():
            action.setChecked(False)
        
        # Označi selektovanu veličinu
    ...
    def initTray(self):
        ...
        # Bazne dimenzije (optimizovano za Full HD @ 96 DPI)
        self.base_width = 420
        self.base_height = 900
        
        # Različite rezolucije sa DPI scaling faktorom
        resolution_presets = {
            "XGA (1024x768)": 0.65,           # 65% scaling - stari CRT monitori
            "WXGA (1280x800)": 0.75,          # 75% scaling - stari laptop
            "HD Ready (1366x768)": 0.80,      # 80% scaling - povećano sa 85%
            "Full HD (1920x1080)": 1.0,       # Baseline - bez scaling-a
            "QHD (2560x1440)": 1.33,          # 133% scaling
            "4K UHD (3840x2160)": 2.0,        # 200% scaling (2x)
            "5K (5120x2880)": 2.67,           # 267% scaling
            "8K UHD (7680x4320)": 3.0         # 300% scaling (3x)
        }
        ...
        tray_menu.addMenu(self.size_menu)  # Dodaj Size submenu
        tray_menu.addSeparator()
    ...
    def toggleClickThrough(self):
        ...
        if self.click_through:
            ...
            # Prvo ukloni Qt atribut
            self.setAttribute(Qt.WA_TransparentForMouseEvents, False)
            
            # Pa tek onda Windows flag
            try:
                import ctypes
                hwnd = int(self.winId())
                GWL_EXSTYLE = -20
                ...
                # Ukloni SAMO WS_EX_TRANSPARENT, ZADRŽI WS_EX_LAYERED za transparentnost
                style = ctypes.windll.user32.GetWindowLongW(hwnd, GWL_EXSTYLE)
                new_style = (style & ~WS_EX_TRANSPARENT) | WS_EX_LAYERED
                ctypes.windll.user32.SetWindowLongW(hwnd, GWL_EXSTYLE, new_style)
                
                # KRITIČNO: Force refresh prozora
                SWP_NOMOVE = 0x0002
                SWP_NOSIZE = 0x0001
                SWP_NOZORDER = 0x0004
                ...
                # Dodatno: Show/Hide trick za potpuni reset
                self.hide()
    ...
    def setWidgetOnlyMode(self):
        reply = QMessageBox.question(None, 'Widget-only Mode', 'Sakriti tray ikonu?\n\nAplikacija će raditi samo kao desktop widget.\nMožeš zatvoriti Widget klikom na X dugme.', QMessageBox.Yes | QMessageBox.No)
    ...
    def restorePosition(self):
        ...
        # Proveri startup status iz Windows registrija
    ...
    def getCityCoordinates(self, city_name):
        try:
            # Ako je srpski grad, dodaj ", Serbia" za precizniji upit
            ...
            if city_name.lower() in serbian_cities:
                search_query = f"{city_name},RS"  # RS = Serbia country code
            ...
            if response.status_code == 200 and response.json():
                ...
                city_map = {
                    ...
                    'Sombor': 'Sombor',
                    'Stari Grad': 'Beograd',
                    'Semlin': 'Zemun',  # Istorijski naziv
                    'Zimony': 'Zemun'   # Mađarski naziv
                }
    ...
    def updateWeather(self):
        try:
            ...
            # Koristi session umesto direktnog requests.get
            ...
            if response.status_code == 200:
                ...
                cloudiness = data['clouds']['all']  # procenat
                
                weather_info = self.translate_weather(data['weather'][0]['description'])
                desc = weather_info[0]  # Tekst
                weather_icon = weather_info[1]  # Emoji
                city = data['name']
                
                # Fix za opštine i istorijske nazive
                city_map = {
                    'Stari Grad': 'Beograd',
                    'Semlin': 'Zemun',
                    'Zimony': 'Zemun',
                ...
                # Ako je i dalje "Serbia" bug, koristi location_data
                ...
                self.wind_label.setText(f"{wind_speed} km/h {wind_direction}")  # SA PRAVCEM!
                ...
                # ========== AIR QUALITY + DETALJNI POLUTANTI ==========
                try:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for day_key in daily_forecasts:
                    ...
                    daily_forecasts[day_key]['desc'] = weather_info[0]  # Tekst
                    daily_forecasts[day_key]['icon'] = weather_info[1]  # Emoji
                
                # Uzmi prvih 5 dana (uključujući DANAS)
